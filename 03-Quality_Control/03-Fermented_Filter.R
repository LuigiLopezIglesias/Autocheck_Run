...
for (chain in c("16S","ITS")) {
 ...
 Filt_Path <- paste0("./Filter_Parameters/",chain,"/")
 ...
 if (chain == "ITS") {
   ...
   write.table(Fungus, file = paste0(opt$Path,opt$date,"/Wineseq/Good_Reads/Fermented/",chain,"/Informs/Good_Fer_",opt$date,"_",chain,".csv"), col.names = TRUE, row.names = FALSE, sep = ",")
 } else {
   Bacteria <- data.frame()
   for(i in as.character(Reads$Sample)) {
     j <- sub('\\..*', '', i)
     ...
     if (is.na(CheckID[3]) == F) {
       if (CheckID[3] == "Must") {
         ...
         Limits <- Limits %>%
           ...
           `colnames<-`(c("Species", "avg_PER", "sd"))
         Num_SP_control <- Limits %>% summarise(num_sp = n())
         mean <- read.csv(paste0(Filt_Path,CheckID[3],"_",F_Date,"/number_Species_",chian,"_",CheckID[3],".csv"))
         MAX <- read.csv(paste0(Filt_Path,CheckID[3],"_",F_Date,"/max_Species_",chain,"_",CheckID[3],".csv"))
         ### Species number analysis
         SpeciesNUM <- ifelse(nrow(Samp) < (mean[2]-mean[3]), 
                              "Low", 
         ...
         if (Num_SP_CM[,1] < (Num_SP_control[,1]/2)) {
           ...
           TXT <- TEXTO(paste0(i), Samp_data, CheckID, chain, opt$date, Reads_Finales, Control, Control_2, Samp, TOP_10, SpeciesNUM, SpeciesMAX, ControlSpecies, Result)
           Bacteria <- rbind(Bacteria,TXT)
         }
       } else {
	 #print("Fermentation with stage")
         TXT <- TEXTO(paste0(i), Samp_data, CheckID, chain, opt$date, Reads_Finales, Control, Control_2, Samp, TOP_10, "NA", "NA", "NA", Controles)
         Bacteria <- rbind(Bacteria,TXT)
       }
     } else {
       print(paste0(i," have unknown stage"))
       TXT <- TEXTO(paste0(i), Samp_data, "NA", chain, opt$date, Reads_Finales, Control, Control_2, Samp, TOP_10, "NA", "NA", "NA", Controles)
       Bacteria <- rbind(Bacteria,TXT)
     }
   }
   dir.create(paste0(opt$Path,opt$date,"/Wineseq/Good_Reads/Fermented/",chain,"/Informs/"), showWarnings = FALSE, recursive = TRUE)
